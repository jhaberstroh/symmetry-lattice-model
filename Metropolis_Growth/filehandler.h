...
class MonteCarloFile : public FileHandler{
 ...
 private:
  ...
  /*--------------------------------------------------
    ...
    --------------------------------------------------*/
 private:
  MonteCarloFile(MonteCarlo* const mc_save = 0, vector<FNameOpt>& fname_include = vector<FNameOpt>(0), ios::openmode write_openmode = ios::out);
  void init_montecarlo(MonteCarlo* const mc_save = 0, vector<FNameOpt>& fname_include = vector<FNameOpt>(0), ios::openmode write_openmode = ios::out);
  /*--------------------------------------------------
    Member Functions
    --------------------------------------------------*/
 protected:
