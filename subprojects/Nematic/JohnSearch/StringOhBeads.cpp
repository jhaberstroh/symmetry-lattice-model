...
StringOhBeads::StringOhBeads(int length,
  ...
  for (int i=0; i<len;i++){
    ...
    default:
      cout << "You dun messed up. Error inside the constructor, no beads initialized.";
      break;
    }    
  }
}
...
void StringOhBeads::string_heun_step(double T, int R,
  ...
  for (int i = 0; i < len ; i++){
    if (mobile[i]) beads[i]->heun_step(T, R, J, Q, Q2);
    cout << beads[i]->getP() << "\n";                     //DEBUG
  }
